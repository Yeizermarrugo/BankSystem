...
export const createTransactionByAccount = async (accountId: string, data: TransactionModel & { accountNumber: string }, userId: string, token: string, notificationService: NotificationPort, userServise: UserPort, logsService: LogsPort): Promise<TransactionModel> => {
	try {
		...
		await logsService.sendLog("transactions", newTransaction.id, "create", "success", `Transacción creada: ${JSON.stringify(newTransaction.dataValues)}`);
		...
		if (data.categoria === "transferencia_envio" && destinoAccountId && data.tipo === "egreso") {
			...
			await logsService.sendLog("transactions", destinoAccountId, "create", "success", `Transacción de ingreso registrada en la cuenta destino`);
			try {
				...
				await logsService.sendLog("transactions", newTransaction.id, "notification", "error", `Error enviando notificación: ${error.message}`);
			}
		}

		return newTransaction;
	} catch (error: any) {
		await logsService.sendLog("transactions", accountId, "create", "error", `Error en transacción: ${error.message}`);
